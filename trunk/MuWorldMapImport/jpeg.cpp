...
bool CJpeg::loadJpegFromBuffer(const char* buffer, size_t size)
{
	lpJpegBuf = (const unsigned char*)buffer;
	...
	LineBytes=(unsigned long)WIDTHbyteS(ImgWidth*24);
	lpPtr = new char[ImgWidth*ImgHeight*24];
...
bool CJpeg::LoadJpegFile(const char* szFilename)
{
	size_t uFileSize;
	...
	fread(buffer,uFileSize,1,f);
	fclose(f);

	bool ret = loadJpegFromBuffer(buffer,uFileSize);
	S_DEL(buffer);
	return ret;
}

char* CJpeg::getBuffer()
{
	return lpPtr;
}

bool CJpeg::saveToBmp(const char* szFilename)
{
	BITMAPFILEHEADER   bf;
	BITMAPINFOHEADER   bi;
	...
	bi.biSize=(unsigned long)sizeof(BITMAPINFOHEADER);
	...
	//NumColors=0;
	bf.bfSize=sizeof(BITMAPFILEHEADER)+sizeof(BITMAPINFOHEADER)+0*sizeof(RGBQUAD)+bi.biWidth*bi.biHeight*bi.biBitCount;
	bf.bfOffBits=54;//(unsigned long)(NumColors*sizeof(RGBQUAD)+sizeof(BITMAPFILEHEADER)+sizeof(BITMAPINFOHEADER));
	...
	if (lpPtr)
	{
		...
		if(f)
		{
			...
			fwrite(lpPtr,ImgWidth*ImgHeight*24,1,f);
			fclose(f);
			return true;
		}
	}
...
bool CJpeg::saveToTxt(const char* szFilename)
{
	if (lpPtr)
	{
		FILE* f=fopen(szFilename,"wb");
		if(f)
		{
			size_t uImgSize = ImgWidth*ImgHeight;
			char * hImgData256 = new char[uImgSize];
			{
				char * temp=hImgData256;
				for(size_t i=0; i < uImgSize; ++i)
				{
					char t3=*lpPtr;
					t3 &= 0xE0;
					char t1=*(lpPtr+1);
			...
			fwrite(hImgData256,uImgSize,1,f);
			delete hImgData256;
			fclose(f);
			return true;
		}
...
//////////////////////////////////////////////////////////////////
int  CJpeg::HufBlock(unsigned char dchufindex,unsigned char achufindex)
{
	short count=0;
	short i;
	int funcret;
...
/////////////////////////////////////////////////////////////////////////////////////
void  CJpeg::IQtIZzMCUComponent(short flag)
{
	short H,VV;
	short i,j;
	int *pQtZzMCUBuffer;
...
//////////////////////////////////////////////////////////////////////////////////////////
void  CJpeg::IQtIZzBlock(short  *s ,int * d,short flag)
{
	short i,j;
	short tag;
	short *pQt;
...
///////////////////////////////////////////////////////////////////////////////
void  CJpeg::Fast_IDCT(int * block)
{
	short i;
...
///////////////////////////////////////////////////////////////////////
void  CJpeg::Initialize_Fast_IDCT()
{
	short i;
