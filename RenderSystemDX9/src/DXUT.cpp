...
inline CD3D9RenderSystem& GetD3D9RenderSystem()
{
	return static_cast<CD3D9RenderSystem&>(GetRenderSystem());
}
// Thread safety 
                          ...
                          HICON hIcon, HMENU hMenu, int x, int y)
{
    ...
    if(DXUTGetHWNDFocus() == NULL)
    {
        ...
        StringCchCopy(strCachedWindowTitle, 256, strWindowTitle.c_str());
...
void DXUTUpdateFrameStats()
{
	...
	double fSleepTime = GetDXUTState().GetSleepTime();
	...
	static double fAllSleepTime = 0;
	fAllSleepTime += fSleepTime;
    // Update the scene stats once per second
    if(fAbsTime - fLastTime > 1.0f)
    {
        float fFPS = (float) (dwFrames / (fAbsTime - fLastTime));
		float fTimePerRenderFrame = (fAbsTime - fLastTime-fAllSleepTime) / (float) dwFrames;
        ...
        StringCchPrintf(pstrFPS, 64, L"%0.2f fps %0.6f f/s ", fFPS, fTimePerRenderFrame);
    }
}
